"""Module to publish arbitrage signal data to RabbitMQ or AWS SQS."""
...
RABBITMQ_ROUTING_KEY = os.getenv("RABBITMQ_ROUTING_KEY", "arbitrage_opportunity")
...
def publish_to_queue(payload: list[dict]) -> None:
    """Publishes arbitrage signal results to RabbitMQ or SQS.

    Args:
        payload (list[dict]): A list of signal message payloads.
...
def _send_to_rabbitmq(data: dict) -> None:
    ...
    try:
        ...
        channel.close()
        connection.close()
        logger.info("✅ Published arbitrage signal to RabbitMQ")
...
def _send_to_sqs(data: dict) -> None:
    ...
    try:
        ...
        logger.info("✅ Published arbitrage signal to SQS, MessageId: %s", response["MessageId"])
    except Exception as e:
